...
def predict_last_value(diff_rows):
    # start at the bottom row and work upwards
    diff_rows.reverse()
    # append a zero to the first row
    first_val_in_row = 0
    for row in diff_rows:
        row.insert(0, row[0] - first_val_in_row)
        first_val_in_row = row[0]

    print("predicted last value: " + str(first_val_in_row))
    return first_val_in_row
...
for row in inputs:
    ...
    while not all(value == 0 for value in new_row):
        ...
        print(new_row)
    extrap_values.append(predict_last_value(diff_rows))
